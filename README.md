...
| Web (HTML5) | `Solitaire_web.zip` — must be served over HTTP (see below) |

> **macOS:** Not notarized — on first launch, right-click → Open to bypass Gatekeeper.
>
> **Web:** Browsers won't run it from `file://`. Unzip and serve the folder, e.g. `python3 -m http.server 8000`, then open `http://localhost:8000/index.html`.
